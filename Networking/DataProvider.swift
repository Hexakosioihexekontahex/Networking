...
class DataProvider: NSObject {
    ...
    private lazy var bgSession: URLSession = {
        let config = URLSessionConfiguration.background(withIdentifier: "ru.hex.Networking")
        config.isDiscretionary = true // Запуск задачи в оптимальгное время (по умолчанию false)
...
extension DataProvider: URLSessionDownloadDelegate {
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        ...
        print("Downloading's finished: \(location.absoluteString)")
